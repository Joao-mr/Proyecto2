<template>
  <div class="game-container">
    
    <div class="top-section">
      <div class="panel panel-left">
        <span class="badge-text">Cómo funciona</span>
        <h2 class="title">¿CÓMO SE JUEGA?</h2>
        
        <ol class="instructions-list">
          <li>Entra en una sala de cualquier categoría</li>
          <li>Aparece una imagen en pantalla</li>
          <li>Escribe lo que crees que es antes que nadie</li>
          <li>Gana puntos y sube en el ranking global</li>
        </ol>
        
        <RouterLink class="action-btn" to="/info">› Ver cómo jugar</RouterLink>
      </div>

      <div class="panel panel-right">
        <span class="badge-text">Competición</span>
        <h2 class="title">Pon a prueba tus<br>conocimientos</h2>
        
        <p class="description">
          Compite en tiempo real contra jugadores de todo el mundo.
          Cuanto más rápido aciertes, más puntos consigues.
          Cada partida cuenta para tu posición en el ranking.
        </p>
        
        <RouterLink class="action-btn" to="/categorias">› Competir ahora</RouterLink>
      </div>
    </div>

    <div class="bottom-section">
      <div class="horizontal-line"></div>
      
      <div class="features-grid">
        
        <div class="feature-item">
          <div class="icon-wrapper shadow-effect">
            <img src="/images/icono1.svg" alt="icono1" class="icon-placeholder" />
          </div>
          <p>Categorías variadas</p>
        </div>
        
        <div class="feature-item">
          <div class="icon-wrapper shadow-effect">
            <img src="/images/icono2.svg" alt="icono2" class="icon-placeholder" />
          </div>
          <p>Ranking global</p>
        </div>
        
        <div class="feature-item">
          <div class="icon-wrapper shadow-effect">
            <img src="/images/icono3.svg" alt="icono3" class="icon-placeholder" />
          </div>
          <p>Responde rápido</p>
        </div>
        
        <div class="feature-item no-border">
          <div class="icon-wrapper shadow-effect">
            <img src="/images/icono4.svg" alt="icono4" class="icon-placeholder" />
          </div>
          <p>Salas en tiempo real</p>
        </div>

      </div>
    </div>

  </div>
</template>

