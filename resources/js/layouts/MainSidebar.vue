<template>
    ...
    >
        <div
            class="d-flex align-items-center justify-content-center p-3 border-bottom flex-shrink-0"
        ...
        <div
            class="d-flex flex-column overflow-y-auto overflow-x-hidden p-2 gap-1 flex-grow-1 scrollbar-hide"
            style="overflow-y: auto; overflow-x: hidden;"
        >
            <template v-for="(item, index) in menuModel" :key="index">
                <div
                    v-if="item.label && item.items && !props.isCollapsed"
    ...
    <div
        ...
        class="d-lg-none position-fixed top-0 start-0 w-100 h-100 sidebar-overlay"
        style="z-index: 40; background: rgba(17,24,39,0.5); backdrop-filter: blur(2px);"
    ></div>
</template>
...
const menuModel = computed(() => {
    ...
    const items = [
        ...
        },
        {
            label: 'Contenido',
            items: [
                { label: 'Categorias', icon: 'pi pi-th-large', route: '/admin/categorias' },
                { label: 'Salas', icon: 'pi pi-users', route: '/admin/salas' },
                { label: 'Partidas', icon: 'pi pi-play-circle', route: '/admin/partidas' },
                { label: 'Imagenes', icon: 'pi pi-image', route: '/admin/imagenes' }
            ]
        }
    ];
    ...
    return items.filter((item) => {
        ...
        if (item.items) {
            item.items = item.items.filter((child) => !child.permission || can(child.permission));
